...
* 非局在化タイブレークによる A* の高速化 (Asai, Fukunaga, AAAI2016)    :6min:
...
*** A* の動作 (おさらい)
 ...
 + 解の最適性: 経路の *コストの和* が最小 → *長さ* はいくらでも長くて良い

 + A*の f によるプラトー: 同じコストを持つノード集合 → 特にプラトー内では、 いくらでも長くて良い

 + *プラトー内の探索順序がなんであれ最適解を発見*

 #+begin_alignright
 #+begin_larger
 + 実は A* は *プラトー内では(経路の長さを気にしない)充足解探索をしている* (充足解探索: 最短でなくて良い経路探索)
 #+end_larger
 #+end_alignright
...
** 下界関数を二個評価する場合を減らすために動的に切り替え
 ...
 + これが役に立つのはプラトーが巨大な場合のみ (特にゼロコスト問題)
...
*** 無限グラフ上での A* の完全性
  ...
  正しくは, *タイブレークのために用いられる充足解探索アルゴリズムの無限グラフでの完全性に依存*
  ...
  + *A*が充足解探索の繰り返しである と再定義することで、 充足解探索の理論的結果を援用可能!*

** まとめと今後の展望

 + A* を *充足解探索の繰り返し* と再定義
 + 最適プランニングに充足解探索を埋め込み、有用性を確認
 + 充足解探索の理論的結果を、最適探索の理論的性質を調べるのに援用
 + 様々な既存の充足解探索手法が存在するため、多くの応用が考えられる
   + プラトー内に制限した *マクロアクション* (cf. Marvin)
   + プラトー内に制限した *モンテカルロ探索* (cf. Arvand)
...
探索非局在化手法は *たくさん* ある
 ...
 + Diverse BFS (Imai, AAAI11)
 + Monte-Carlo Random Walk (MRW) --- 一定の確率でランダムウォーク
 + Epsilon-greedy GBFS --- *alt* ([h,*], [Random]) --- 時々ノードを乱択
 + Type-based GBFS --- *alt* ([h,*], <g,h>) --- 時々 h をランダムに選ぶ
...
** 既存の非局在化手法の背景にある考え : Heuristic Error
 ...
 $h$ : 下界値, $h^*$ : パーフェクトヒューリスティクス (解までの真の最短経路コスト)
...
*** プラトー内 error & 非局在化
 ...
 #+end_span6
 #+begin_span6
 + *最小の $h$ を持つノードの中にも 大きな $h^*$ を持つノードが混じっている*
 + *決定的なタイブレーク (FIFO/LIFO) はたまたま 大きな $h^*$ ばかり選択する可能性がある*
 + *プラトー内で探索を非局在化する* 事で *小さな $h^*$ を引く可能性を高める*
 #+end_span6
 #+end_row-fluid
 #+end_container-fluid
...
+ *目的: 同じ非局在化手法を プラトー内とプラトー外で用いることで、プラトー内/外 error が独立であることを示す*

+ *Type-GBFS* (プラトー外) と *RandomDepth Tiebreaking* (プラトー内)

  + Type GBFS --- *Randomize <g,h>*

  + RandomDepth --- *プラトー内で Randomize <d>*
...
*** プラトー内/外 error は 独立
 ...
 #+end_span6
 #+begin_span6
 + hd (RandomDepth) :: プラトー内で Randomize <d>
 + hD (Type GBFS) :: Randomize <g,h>
 + hdD :: */RandomDepth + Type GBFS/*

 結果:

 + ドメインごとに プラトー内 と プラトー外 非局在化 の影響の有無が 綺麗に別れる
 + *同時に使うことで /効果が綺麗に重なる/*
 + _/プラトー内外は独立だから、非局在化手法は、内外に同時に使うべし!/_
 #+end_span6
 #+end_row-fluid
 #+end_container-fluid

* 新たな非局在化手法の開発 (Asai, Fukunaga, ICAPS 2017)                :4min:
...
** Evaluation
 ...
 #+begin_span6
 + *hb* : *プラトー内BIP* ([h, BIP, *])
 + *hB* : *プラトー外BIP* (*alt* ([BIP], [h,*]))
 + *hbB* : *同時使用* (*alt* ([BIP], [h,BIP,*]) )
 + *hdD* : *RandomDepth + Type GBFS*
 + _/プラトー内外errorの直交性/_ を再び観測
...
*** VS. State-of-the-Art
 ...
 #+end_span7
 #+begin_span5
 + db^2 DB : プラトー外に D,B を切り替え ; プラトー内に d,b を切り替え
