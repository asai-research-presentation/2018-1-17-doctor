...
[[png:graphsearch-purpose]]

** 古典プランニング問題 (決定的,完全情報) -- Blocksworld               :3min:
...
** Iterative Depth: RandomDepth の決定的バージョン                 :noexport:
...
** 結果: /A*の性能をタイブレーキングを変えるだけで改善!!/
 ...
 |-------------------------+-------------------------------+-----------------|
 | IPC^1 Instances (1104)  | 558                           | *571.4*         |
...
A*の完全性に関する既存の証明は ゼロコスト問題では成り立たない

 Hart et al. (1968): [[png:hart-cost]]
...
+ A* を *充足解探索の繰り返し* と再定義
  + この視点の利点:
    + *最適解探索に充足解探索を埋め込める*: 実験でも性能向上
    + *充足解探索の理論的性質* を、 *最適探索の理論的性質を調べる* のに利用
...
** State of the Art に対して性能向上 (論文、表7.5.1)
...
|              |       |              |            | (内外)        |
|--------------+-------+--------------+------------+---------------|
| 制限時間内に | 293.2 | 295.4        | 297.6      | *304.5*       |
| 解けた問題数 |       |              |            |               |
