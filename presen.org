...
+ タイブレークによる A* の高速化 (AAAI2016) (JSAI 学生奨励賞)

+ 最適解探索のタイブレークの一般化 (JAIR 2017)

+ 充足解探索の タイブレーク手法 と 探索非局在化手法 の統合 (ICAPS 2017)

+ 新たな非局在化手法 (ICAPS 2017)
...
+ *探索非局在化手法* とは ::
     ...
     *非許容的 h* → *時折デタラメな値を返す* 
     ...
     GBFSはhを貪欲に信頼 → *一定確率で無視して最悪ケースを回避, 探索を効率化*
